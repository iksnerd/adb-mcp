...
- [x] **Code coverage reporting.** Shipped: `get_coverage_report`/`get_file_coverage` run the JaCoCo report task (default `jacocoTestReport`) and parse its XML output — overall line/branch/method/class coverage plus a worst-covered-package-first breakdown, and per-file line/method detail (`file` matches by suffix). Scoped to **JVM unit-test coverage only**; instrumented (on-device) coverage via AGP's `.ec` binary format was deliberately left out — it needs merging execution data with class files and is far more AGP-version-dependent, and no field report has asked for it. Requires the project to already apply the jacoco Gradle plugin with XML reporting enabled; the tool errors with that pointer if no report is found. A multi-module build emits one report per module and `gradlew jacocoTestReport` runs all of them, so every module's report is parsed and merged into one set of totals, with the merged file list shown in the output; within a single module only the newest report is used, so a module that also emits per-variant reports (`jacocoTestDebugUnitTestReport` alongside `jacocoTestReleaseUnitTestReport`) can't double-count the same classes. **Both report locations are read** (v0.22.1): a hand-written `JacocoReport` task writes to `build/reports/jacoco/`, but AGP's own `createDebugUnitTestCoverageReport` — what `enableUnitTestCoverage = true` gives you, with no `jacoco` plugin block in sight — writes the same JaCoCo DTD 1.1 document to `build/reports/coverage/`; matching only the former meant the more common setup ran its task successfully and was then told no report existed. Verified via unit tests against synthetic JaCoCo XML (percentages, worst-first package sort + cap, suffix/ambiguous file matching, line-range collapsing, cross-module merge with a package present in two modules) and live end-to-end (stdio JSON-RPC round-trip against a scaffolded project with the jacoco plugin added: `run_unit_tests` → `get_coverage_report` text and json=true → `get_file_coverage` for a matched, suffix-matched, and not-found file, coverage numbers hand-verified against the source; then again against a two-module build, where the merged totals equal the sum of the per-module reports and a file in the second module resolves).
- [x] **Session defaults.** Shipped: `session_set_defaults`/`session_show_defaults`/`session_clear_defaults` let an agent pin `project_dir` and/or `serial` for the rest of the session, so Gradle and device tools can omit them. XcodeBuildMCP's `session_use_defaults_profile` (named profiles) was left out — no per-tool default-*task* was added either, since "task" means different things per tool (build variant vs. test target vs. list-scope) and guessing an AGP task-naming convention from a variant name risked silently picking the wrong task on a nonstandard build; only `project_dir`/`serial`, which are repeated verbatim, are defaulted. Verified live (stdio JSON-RPC round-trip: set → show → an omitted-`project_dir` call resolves to the pinned value → clear → the same call requires it again).

## Enhancements

- [x] **`scaffold_android_project`'s generated project didn't compile standalone.** Found 2026-08-16 verifying JaCoCo coverage reporting. Two bugs, both fixed: (1) the generated Activity extends `androidx.activity.ComponentActivity`, but `app/build.gradle.kts` never declared the `androidx.activity` dependency — `compileDebugKotlin`/`compileReleaseKotlin` failed with "Unresolved reference 'ComponentActivity'"; now declares `implementation("androidx.activity:activity-ktx:1.9.3")`. (2) no `compileOptions`/`kotlinOptions` were set, so on a machine whose default JDK is 17 (increasingly the common case), javac's implicit 1.8 target and kotlinc's implicit 17 target (it matches the running JDK) collide with "Inconsistent JVM-target compatibility"; now both are pinned to 17. Verified live: a freshly scaffolded, completely unmodified project now runs `gradle_build` (assembleDebug) to a successful APK.
...
- [x] **Accessibility-action tap for native surfaces (the real fix) — EXPERIMENTAL, shipped v0.21.0.** Maestro's `tapOn` navigated the same bar first try — it clicks through UiAutomator (`AccessibilityNodeInfo.performAction(ACTION_CLICK)`), which reaches views that a low-level `input tap` coordinate event doesn't (Compose/RN `NativeTabs`, some overlays). Confirmed there is no adb-shell-only equivalent (no `uiautomator` action subcommand, no `cmd accessibility` action verb, raw `service call accessibility` binder calls too fragile to maintain across Android versions) — a companion on-device app is unavoidable, but it can be far lighter than Maestro's two-APK + `am instrument` gRPC server: a plain `AccessibilityService` (repo-root `bridge/`, see [bridge/README.md](../bridge/README.md)) gets the full node tree with no instrumentation overhead. One-time `adb-mcp bridge install` per device; then `tap_on_text`/`tap_element` accept `via_accessibility=true`. Verified live end-to-end (built binary, real MCP stdio calls, text and resource_id matching, no-match handling) on `emulator-5556`. Caveat: this proves the mechanism works, not the specific "coordinate tap no-ops, accessibility click succeeds" differential from the original field report — that needs the reporter's actual `NativeTabs` app, which wasn't available for this pass.
