...
- [x] **App/bundle state probe (the most expensive gap).** **Shipped v0.16.0 as `app_state`.** Reports installed?/running? + pid(s), main-process uptime (`ps ETIME`), first-install/last-update times (`dumpsys package`), and a Metro-vs-embedded bundle verdict from recent logcat markers (`HMRClient`/`Fast Refresh`/`DevServerHelper` → metro; summed with the evidence line it keyed on). Flags multiple live processes for one package (the "presses and log-reads hit different pids" trap). Verified live against system apps; the bundle heuristic is unit-tested with synthetic RN logs.
