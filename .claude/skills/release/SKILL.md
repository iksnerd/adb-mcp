...
## 4. Sync the counts (`make docs`)

Run it after the version bump — it reads `VERSION` and the code, and rewrites
every derived number:

```bash
make docs        # rewrite the counts
make docs-check  # verify only; what the release gate runs
```

It covers the tool/guide/area counts in `README.md` and `docs/TOOLS.md`, the
`**Current:** vX.Y.Z · N tools + M guide resources` banner and the CHANGELOG
range in the gitignored `docs/personal/TODO.md`, and it fails if a registered
tool is missing from `docs/TOOLS.md` entirely. `docs-check` now runs in the
release gate, so the tracked files can't ship stale; the roadmap is local-only
and updated whenever you run `make docs`.

These numbers were hand-typed for many releases and were wrong more than once —
if you catch yourself editing one by hand, that's the bug.

**Still manual in `docs/personal/TODO.md`** (prose, not counts): the "Recently
shipped (vX.Y.Z)" heading *and its body*, and any parity checkboxes the release
closes. Write the body at the end, once you know what the release actually
became — it grows during the work, and a summary written early goes stale.
...
make docs-check       # tool/guide/area counts still agree with the code
...
| Tool count wrong in docs | Hand-edited instead of running `make docs` — never type these numbers |
